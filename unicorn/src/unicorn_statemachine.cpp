	...
	: move_base_clt_("move_base", true)
{
	...
	unicorn_state_pub_ = n_.advertise<std_msgs::Int32>("/TX2_unicorn_state", 0);
	// unicorn_state_sub_ = n_.subscribe("/TX2_unicorn_state", 0, &UnicornState::stateCallback, this);
	...
	updateAndPublishState(current_state::MANUAL);
...
void UnicornState::stateCallback(const std_msgs::Int32 &msg)
{
	std::cout << "Received the following state: " << msg.data << std::endl;
	return;
}
